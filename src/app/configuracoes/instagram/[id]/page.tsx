...
import { db } from "@/db";
import { instagramChannelRestrictions, instagramChannels, users } from "@/db/schema";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { EmptyState } from "@/components/ui/empty-state";
import { AccessToggle } from "./access-toggle";

export default async function InstagramChannelAccessPage({
  params,
}: {
  ...
  const [channel] = await db
    ...
    .where(eq(instagramChannels.id, id))
    .limit(1);
  if (!channel) notFound();
  ...
  const restrictions = await db
    ...
    .where(eq(instagramChannelRestrictions.channelId, id));
